  ...
  return (
    <BlurFade delay={0.25} inView>
      <div className="mt-10">
        ...
        {Array.from(groupedCareers.entries()).map(
          ([companyName, companyCarers]) => (
            <div key={companyName} className="relative">
              {companyCarers.length > 1 && (
                <div className="absolute left-[31px] top-[72px] bottom-4 w-[2px] bg-muted-foreground/30" />
              )}

              {companyCarers.map((career: any, idx: number) => (
                <div key={career.id} className="relative">
                  {idx > 0 && (
                    <div className="absolute left-[28px] top-[26px] w-2 h-2 rounded-full bg-foreground z-10" />
                  )}
                  <div className="prose max-w-full text-pretty font-sans text-sm dark:prose-invert mt-2 flex flex-row gap-4">
                    {idx === 0 ? (
                      <FallbackImage
                        ...
                        width={64}
                        height={64}
                        className="rounded-full object-cover size-16 flex-shrink-0"
                      />
                    ) : (

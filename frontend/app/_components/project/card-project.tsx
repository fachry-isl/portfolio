      ...
      <Link href={`/project/${id}`} className="block overflow-hidden">
        ...
        <CardHeader className="p-4 pb-2">
          <div className="space-y-1.5">
            ...
            <div className="prose max-w-full text-pretty font-sans text-sm text-muted-foreground dark:prose-invert line-clamp-2 leading-snug">
              {description}
            </div>
          </div>
        </CardHeader>
          ...
          {href && href.length > 0 && (
            <Link href={href} target="_blank">
              <Button
                size="sm"
                variant="secondary"
                className="h-8 gap-1.5 rounded-md hover:bg-primary hover:text-primary-foreground transition-colors cursor-pointer"
              >
          ...
          {source && source.length > 0 && (
            <Link href={source} target="_blank">
              <Button
                size="sm"
                variant="outline"
                className="h-8 gap-1.5 rounded-md hover:bg-primary/10 transition-colors cursor-pointer"
              >
